...
帧协议（与 serial_controller.py 一致）:
    [0xAA] [0x55] [pan] [tilt] [track_id] [checksum], checksum = (pan+tilt+tid) & 0xFF
"""
...
def main():
    ...
    # 帧解析状态机：HEAD1 → HEAD2 → PAN → TILT → ID → CHK
    state = "HEAD1"
    pan = tilt = tid = 0
    last_print = 0.0
    frames = 0
                ...
                if b == expected:
                    ...
                    if now - last_print >= 0.03:   # 限制刷新率 ~33Hz
                        ...
                        print(f"\r水平:{pan:3d}° {bar_h} | 俯仰:{tilt:3d}° {bar_v}"
                              f" | ID:{tid:3d} [帧:{frames} 丢:{drops}]", end="")
                        last_print = now
                else:
                    drops += 1
                state = "HEAD1"
